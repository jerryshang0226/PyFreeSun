...
from frm_popup_Supppiler import *
from dbtool import *
from main import *

# 窗口初始化
class main(QMainWindow, Ui_popup_Suppiler):
    def __init__(self):
        super(main, self).__init__()
        self.setupUi(self)
        self.query_suppiler()

    def query_suppiler(self):
        self.tableWidget.setRowCount(0)
        self.tableWidget.setColumnCount(5)
        self.tableWidget.setHorizontalHeaderLabels(['供应商Id', '供应商名称', '行业', '规模', '省份'])
        self.tableWidget.horizontalHeader().setStretchLastSection(True)
        # 绑定数据
        result = dbQueryFull(
            "select id,Name,Industry,Scale,Place from S_ACCOUNT where id != 0 and Active_Flg = 'Y' and Supppiler_Flg = 'Y'")
        row = len(result)
    ...
    def click_pbtn_pick(self):
        ...
        # 如果有选中记录
        if row_index >= 0:
            globalvar.pick_suppiler_id = self.tableWidget.item(row_index, 0).text()
            self.close()
        else:
...
def popup_pick(main_window,popup_win):
    ...
    # 如果有选中记录
    if row_index >= 0:
        globalvar.pick_suppiler_id = popup_win.tableWidget.item(row_index, 0).text()
        popup_win.close()
